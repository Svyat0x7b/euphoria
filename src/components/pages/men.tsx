...
const Men = () => {
    ...
    const [actualEndpoint, setActualEndpoint] = useState(`${API_DOMAIN}/api/v1/products/men`);
    ...
    const applyFiltersHandler = (
        ...
        let endpoint = `${API_DOMAIN}/api/v1/products/men`;
        let queryParams = [];

        if (color.length) {
            queryParams.push(`color=${color.toLowerCase()}`);
        }
        if (size.length) {
    ...
    return (
        <main className="px-[100px] pb-[100px] flex w-full">
            ...
            {!error.is ? (
                <ProductList title="Men Clothing" products={fetchedProducts} />
            ) : (
